"""
New session-based authentication routes for DebtEase application.
...
async def login_form(
    ...
    Supports both session cookies and bearer token response.
    """
    ...
    # Use the main login logic
    auth_response = await login(credentials, response, request)
    
    # Also return token format for OAuth2 compatibility
    session_data = AuthUtils.get_session(response.headers.get('Set-Cookie'))
    
    return {
        "access_token": "session_based",  # Placeholder for OAuth2 compatibility
        "token_type": "session",
        "user": auth_response.user,
        "expires_at": auth_response.session_expires_at
    }
